...
class Agent():
    def __init__(self, env, num_frames, batch_size, gamma, replay_initial, model_dir, fig_dir, p_dir):
        ...
        self.current_model = CnnDQN(self.obs_dim, self.act_dim, self.env)
        self.target_model = CnnDQN(self.obs_dim, self.act_dim, self.env)
    ...
    def train(self):
        ...
        for frame_idx in pbar:
            ...
            if frame_idx % 50000 == 0:
                ...
                # 保存图片
                figure.savefig(self.p_dir + 'ddqn_' + '%d.png' % frame_idx)
                # 保存绘图数据
                reward_tmp = np.array(all_rewards)
                losses_tmp = np.array(losses)
                np.save(self.fig_dir + 'ddqn_reward_' + '%d.npy' % frame_idx, reward_tmp)  # 保存为.npy格式
                np.save(self.fig_dir + 'ddqn_loss_' + '%d.npy' % frame_idx, losses_tmp)  # 保存为.npy格式
